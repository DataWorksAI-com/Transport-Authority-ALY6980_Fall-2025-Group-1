...
# NANDA Registry - Linode Docker Deployment with Infrastructure Provisioning
# This script creates a Linode instance and deploys the registry using Docker Compose

set -e

# Configuration - Override with environment variables
REGION="${REGION:-us-east}"
INSTANCE_TYPE="${INSTANCE_TYPE:-g6-standard-2}"  # 4GB RAM recommended for Docker
INSTANCE_LABEL="${INSTANCE_LABEL:-nanda-registry-docker}"
...
if [ -z "$ATLAS_URL" ]; then
    ...
    echo ""
    echo "Usage: ATLAS_URL=your_mongodb_url ./provision-and-deploy-docker.sh"
    echo ""
    echo "Optional environment variables:"
    echo "  REGION           - Linode region (default: us-east)"
    echo "  INSTANCE_TYPE    - Linode plan (default: g6-standard-2)"
    echo "  INSTANCE_LABEL   - Instance label (default: nanda-registry-docker)"
    echo "  ROOT_PASSWORD    - Root password (auto-generated if not provided)"
    echo ""
    echo "Common regions: us-east, us-west, eu-west, ap-south"
    echo "Common types: g6-standard-1 (2GB), g6-standard-2 (4GB), g6-standard-4 (8GB)"
    exit 1
fi
...
log_info "NANDA Registry - Docker Deployment with Infrastructure Provisioning"
echo "=========================================================================="
echo "Region: $REGION"
...
log_step "7/8" "Installing Docker and deploying application..."
...
scp -i "$SSH_KEY_LABEL" -o StrictHostKeyChecking=no Dockerfile "root@$PUBLIC_IP:/opt/nanda-registry/"
scp -i "$SSH_KEY_LABEL" -o StrictHostKeyChecking=no docker-compose.yml "root@$PUBLIC_IP:/opt/nanda-registry/"
...
echo "  ssh -i ${SSH_KEY_LABEL} root@$PUBLIC_IP 'cd /opt/nanda-registry && docker-compose logs -f'"
echo "  ssh -i ${SSH_KEY_LABEL} root@$PUBLIC_IP 'cd /opt/nanda-registry && docker-compose restart'"
echo ""
